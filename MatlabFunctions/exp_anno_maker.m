...
function exp_anno_maker(batch, plate_DB, year)
year = 2017
batch = '2017018_001'
...
info(8, 1:2) =  {'Number of Sheets', 11};
info(9, 1:1) =  {'Sheet Fields'};
info(9, 2:12) = {'UsedWells'	'Compound_ID'	'Compound_Usage'	'Compound_Category'	'Dose_Category'	'Dose'	'Cell_Line'	'CAS_Number'	'Target'	'Pathway'	'Description'};
        ...
        for x=2:17;
            for y=2:25;
                ...
                if (length(uniwell)>1)
                    ...
                    try
                        exp_plate{i+1}{x,y} = thiswell{uniwell};% plus one to shift it along one to allow to add in the usedwells after
